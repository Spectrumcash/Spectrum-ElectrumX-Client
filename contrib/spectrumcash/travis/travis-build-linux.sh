...
mkdir -p electrum-spectrumcash/dist
wget -O electrum-spectrumcash/dist/tor-proxy-setup.exe \
    https://github.com/zebra-lucky/tor-proxy/releases/download/0.3.5.8/tor-proxy-0.3.5.8-setup.exe


docker run --rm \
    -v $(pwd):/opt \
    -w /opt/electrum-spectrumcash \
    -t anonymouszar/electrum-spectrumcash-winebuild:LinuxPy36 /opt/build_linux.sh
...
docker run --rm \
    -v $(pwd):/opt \
    -w /opt/electrum-spectrumcash/contrib/spectrumcash/travis \
    -t anonymouszar/electrum-spectrumcash-winebuild:LinuxAppImage ./build_appimage.sh
...
docker run --rm \
    ...
    -e PYHOME=$PYHOME \
    -v $(pwd):/opt \
    -v $(pwd)/electrum-spectrumcash/:$WINEPREFIX/drive_c/electrum-spectrumcash \
    -w /opt/electrum-spectrumcash \
