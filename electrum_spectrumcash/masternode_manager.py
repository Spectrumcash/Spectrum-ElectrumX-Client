...
class MasternodeManager(object):
    ...
    def get_masternode_outputs(self, domain = None, exclude_frozen = True):
        ...
        correct_amount = lambda d: d['value'] == 15000000 * bitcoin.COIN

        # Valid outputs have a value of exactly 15000000 SPECTRUMCASH and
    ...
    def check_can_sign_masternode(self, alias):
        ...
        if mn.vin.get('value', 0) != bitcoin.COIN * 15000000:
            raise Exception('Masternode requires a collateral 15000000 SPECTRUMCASH output.')
    ...
    def send_announce(self, alias):
        ...
        res = await self.wallet.network.masternode_announce_broadcast(mn)
        self.broadcast_announce_callback(alias, errmsg, res)
        self.subscribe_to_masternodes()
        if errmsg:
            errmsg = errmsg[0]
