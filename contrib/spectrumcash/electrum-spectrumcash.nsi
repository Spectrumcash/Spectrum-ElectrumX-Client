...
;Variables
  ...
  !define PRODUCT_NAME "SpectrumCash Electrum"
  !define PRODUCT_NAME_NO_SPACE "SpectrumCash-Electrum"
  !define PREV_PROD_NAME "Electrum-SPECTRUMCASH"
  !define PREV_PROD_NAME2 "SpectrumCash-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/anonymouszar/electrum-spectrumcash"
...
;Interface Settings
  ...
  !define MUI_ICON "electrum_spectrumcash\gui\icons\electrum-spectrumcash.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\spectrumcash"
  ...
  File /r "dist\electrum-spectrumcash\*.*"
  File "electrum_spectrumcash\gui\icons\electrum-spectrumcash.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spectrumcash-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spectrumcash-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-spectrumcash-${PRODUCT_VERSION}.exe" 0
  

  ;Links spectrumcash: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\spectrumcash" "" "URL:spectrumcash Protocol"
  WriteRegStr HKCU "Software\Classes\spectrumcash" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\spectrumcash" "DefaultIcon" "$\"$INSTDIR\electrum-spectrumcash.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\spectrumcash\shell\open\command" "" "$\"$INSTDIR\electrum-spectrumcash-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-spectrumcash.ico"
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "SpectrumCash Electrum Wallet"
