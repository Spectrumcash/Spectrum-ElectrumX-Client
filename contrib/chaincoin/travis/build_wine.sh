...
cd $WINEPREFIX/drive_c/electrum-chaincoin

rm -rf build
rm -rf dist/electrum-chaincoin

cp contrib/chaincoin/deterministic.spec .
...
wine pip install -r contrib/chaincoin/requirements-win.txt
...
wine pyinstaller -y \
    --name electrum-chaincoin-$ELECTRUM_CHAINCOIN_VERSION.exe \
    deterministic.spec
...
wine "$NSIS_EXE" /NOCD -V3 \
    /DPRODUCT_VERSION=$ELECTRUM_CHAINCOIN_VERSION \
    /DWINEARCH=$WINEARCH \
    contrib/chaincoin/electrum-chaincoin.nsi
