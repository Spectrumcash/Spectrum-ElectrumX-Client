...
# try to make console log lines short... no timestamp, short levelname, no "electrum_spectrumcash."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_spectrumcash."):
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_spectrumcash."):
        name = name[9:]
...
def configure_logging(config):
    ...
    _logger.info(f"SpectrumCash-Electrum version: {ELECTRUM_VERSION} - https://electrum_spectrumcash.org - https://github.com/spectrumcash/electrum-spectrumcash")
