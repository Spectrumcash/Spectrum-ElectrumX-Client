...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-CHAINCOIN"
  !define PRODUCT_WEB_SITE "https://github.com/akhavr/electrum-chaincoin"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-chaincoin-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-chaincoin.ico"
...
Section
  ...
  File /r "dist\electrum-chaincoin\*.*"
  File "icons\electrum-chaincoin.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe" 0


  ;Links chaincoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\chaincoin" "" "URL:chaincoin Protocol"
  WriteRegStr HKCU "Software\Classes\chaincoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\chaincoin" "DefaultIcon" "$\"$INSTDIR\electrum-chaincoin.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\chaincoin\shell\open\command" "" "$\"$INSTDIR\electrum-chaincoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-chaincoin.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\chaincoin"
