...
from electrum_spectrumcash.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_spectrumcash import bitcoin
from electrum_spectrumcash .transaction import TxOutput
from electrum_spectrumcash.util import send_exception_to_crash_reporter
...
from electrum_spectrumcash.wallet import InternalAddressCorruption

from .context_menu import ContextMenu


from electrum_spectrumcash.gui.kivy.i18n import _
...
class CScreen(Factory.Screen):
    ...
    def load_screen(self):
        self.screen = Builder.load_file('electrum_spectrumcash/gui/kivy/uix/ui_screens/' + self.kvname + '.kv')
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_hash, tx_type, tx_mined_status, value, balance):
        ...
        icon = "atlas://electrum_spectrumcash/gui/kivy/theming/light/" + TX_ICONS[status]
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            uri = electrum_spectrumcash.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a SpectrumCash URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_spectrumcash.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a SpectrumCash address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid SpectrumCash Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_spectrumcash.util import create_bip21_uri
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share SpectrumCash Request"))
